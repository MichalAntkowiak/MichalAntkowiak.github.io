...
MAWORLD.buildGroundWorld=function(THREE,opts){
  ...
  const GLASS=new THREE.MeshStandardMaterial({color:0x7fb5ae,roughness:.15,metalness:.4,transparent:true,opacity:.55});
  ...
  const addWin=(w,h,d,x,y,z)=>{const m=new THREE.Mesh(winBox(w,h,d),WINMAT);
    m.position.set(x,y,z);m.userData.win=true;G.add(m);return m};
  ...
  function greenTerraces(x,z,w,d,h){
    ...
    roof.position.set(x,h+.25,z);G.add(roof);
    putDecid(x-w*.25,h+.4,z,.6);putDecid(x+w*.2,h+.4,z+d*.2,.5)}
  function towerGreen(x,z,w,d,h){addWin(w,h,d,x,h/2,z);greenTerraces(x,z,w,d,h);
  ...
  function towerCurved(x,z,r,h){
    const cyl=new THREE.Mesh(new THREE.CylinderGeometry(r,r*1.12,h,16),GLASS);
    cyl.position.set(x,h/2,z);G.add(cyl);
    const cap=new THREE.Mesh(new THREE.SphereGeometry(r,16,10,0,Math.PI*2,0,Math.PI/2),WHITE);
    cap.position.set(x,h,z);G.add(cap);
    for(let k=0;k<11;k++){const a=k*.62,vy=1.5+k*(h-3)/11;
      const v=new THREE.Mesh(new THREE.BoxGeometry(.5,h/11,1.1),GREEN);
  ...
  function midStepped(x,z,w,d,h){
    ...
    [.5,.32,.18].forEach(fr=>{const hh=h*fr;
      ...
      g.position.set(x,y0+hh+.15,z);G.add(g);
      y0+=hh;cw*=.72;cd*=.72});
  ...
  // TRAMWAJ eco: biel + zieleń
  const tram=new THREE.Group();tram.userData.dyn=true;G.add(tram);
  {const wht=std(0xf0f2f1,.5,.1),grn=std(0x6fae5c,.5,.15),
     glz=std(0x1a2730,.15,.7),drk=std(0x14181c,.8);
   const segL=3.1,gap=.16;
   for(let sgi=-1;sgi<=1;sgi++){
     const seg=new THREE.Group();seg.position.x=sgi*(segL+gap);tram.add(seg);
     const skirt=new THREE.Mesh(new THREE.BoxGeometry(segL,.4,1.05),grn);
     skirt.position.y=.5;seg.add(skirt);
     const band=new THREE.Mesh(new THREE.BoxGeometry(segL,.5,1.02),glz);
     band.position.y=.98;seg.add(band);
     const upr=new THREE.Mesh(new THREE.BoxGeometry(segL,.2,1.05),wht);
     upr.position.y=1.33;seg.add(upr);
     const roofl=new THREE.Mesh(new THREE.BoxGeometry(segL*.96,.1,.9),grn);
     roofl.position.y=1.48;seg.add(roofl);
     const bog=new THREE.Mesh(new THREE.BoxGeometry(segL*.5,.3,.8),drk);
     bog.position.y=.18;seg.add(bog);
     if(sgi!==1){const bell=new THREE.Mesh(new THREE.BoxGeometry(gap+.24,.9,.9),drk);
       bell.position.set(sgi*(segL+gap)+(segL+gap)/2,1,0);tram.add(bell)}}
   [-1,1].forEach(e=>{
     const nose=new THREE.Mesh(new THREE.BoxGeometry(.5,.92,.98),wht);
     nose.position.set(e*(1.5*(segL+gap)+.2),.86,0);nose.rotation.z=-e*.2;tram.add(nose);
     const shield=new THREE.Mesh(new THREE.BoxGeometry(.1,.5,.9),glz);
     shield.position.set(e*(1.5*(segL+gap)+.44),1.1,0);shield.rotation.z=-e*.2;tram.add(shield);
     const lamp=new THREE.Mesh(new THREE.BoxGeometry(.06,.09,.2),bas(0xfff2c9));
     lamp.position.set(e*(1.5*(segL+gap)+.5),.6,.28);tram.add(lamp);
     const l2=lamp.clone();l2.position.z=-.28;tram.add(l2)});
   ...
   tram.position.set(-30,0,-14);tram.userData.min=-44;tram.userData.max=44;tram.userData.v=4.2}
  {const pM=std(0x39434d,.6,.3);
   for(let x=-44;x<=44;x+=14.6){
     const pole=new THREE.Mesh(new THREE.CylinderGeometry(.07,.09,4.4,8),pM);
     pole.position.set(x,2.2,-19);G.add(pole);
     const arm=new THREE.Mesh(new THREE.CylinderGeometry(.03,.03,5.2,6),pM);
     arm.rotation.x=Math.PI/2;arm.position.set(x,4.15,-16.5);G.add(arm)}
   const wire=new THREE.Mesh(new THREE.CylinderGeometry(.016,.016,92,6),std(0x20262c,.4,.6));
  ...
  // P-07
  const ringP=new THREE.Mesh(new THREE.TorusGeometry(1.35,.045,10,44),bas(0xe8500f));
  ringP.rotation.x=-Math.PI/2;ringP.position.y=.022;G.add(ringP);
  const hM=bas(0xe8ecef);
  const h1=new THREE.Mesh(new THREE.BoxGeometry(.16,.012,1),hM);h1.position.set(-.3,.024,0);
  const h2=h1.clone();h2.position.x=.3;
  const h3=new THREE.Mesh(new THREE.BoxGeometry(.46,.012,.16),hM);h3.position.y=.024;
  G.add(h1,h2,h3);
  ...
  return {group:G,beacons,tram,spins};
};
...
MAWORLD.mergeStatic=function(THREE,root){
  ...
  if(buckets.glass){const gm=new THREE.Mesh(concat(buckets.glass),
    new THREE.MeshStandardMaterial({vertexColors:true,transparent:true,opacity:.55,roughness:.15,metalness:.4}));
    gm.renderOrder=2;root.add(gm)}

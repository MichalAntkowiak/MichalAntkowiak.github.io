...
MAWORLD.buildGroundWorld=function(THREE,opts){
  ...
  // P-07: uniesiony VERTIPORT — okrągła platforma z podświetloną krawędzią
  {const padGlow=new THREE.MeshBasicMaterial({color:0x7fe6ff});
   const padDeck=std(0x2c3540,.5,.3), padTrim=std(0xdfe4e8,.5,.2);
   // podstawa platformy (lekko uniesiona)
   const base=new THREE.Mesh(new THREE.CylinderGeometry(3.4,3.7,.5,32),padDeck);
   base.position.set(0,.25,0);G.add(base);
   // górna tafla
   const deck=new THREE.Mesh(new THREE.CylinderGeometry(3.2,3.2,.12,32),std(0x374250,.6,.25));
   deck.position.set(0,.52,0);G.add(deck);
   // podświetlony pierścień krawędziowy
   const edge=new THREE.Mesh(new THREE.TorusGeometry(3.2,.08,10,48),padGlow);
   edge.rotation.x=-Math.PI/2;edge.position.set(0,.58,0);G.add(edge);
   // 12 lampek obwodowych (touchdown lights)
   for(let k=0;k<12;k++){const a=k/12*Math.PI*2;
     const l=new THREE.Mesh(new THREE.SphereGeometry(.08,8,6),padGlow);
     l.position.set(Math.cos(a)*2.9,.62,Math.sin(a)*2.9);G.add(l)}
   // pomarańczowy krąg celowniczy + H
   const ringP=new THREE.Mesh(new THREE.TorusGeometry(1.5,.06,10,44),bas(0xe8500f));
   ringP.rotation.x=-Math.PI/2;ringP.position.y=.6;G.add(ringP);
   const hM=bas(0xe8ecef);
   const h1=new THREE.Mesh(new THREE.BoxGeometry(.22,.02,1.2),hM);h1.position.set(-.38,.61,0);
   const h2=h1.clone();h2.position.x=.38;
   const h3=new THREE.Mesh(new THREE.BoxGeometry(.54,.02,.22),hM);h3.position.y=.61;
   G.add(h1,h2,h3);
   // 4 słupki nawigacyjne ze światłem na narożach
   [[2.3,2.3],[-2.3,2.3],[2.3,-2.3],[-2.3,-2.3]].forEach(([px,pz])=>{
     const post=new THREE.Mesh(new THREE.CylinderGeometry(.06,.08,1.4,8),padTrim);
     post.position.set(px,1.2,pz);G.add(post);
     const cap=new THREE.Mesh(new THREE.SphereGeometry(.11,10,8),padGlow);
     cap.position.set(px,1.95,pz);G.add(cap)})}
...
MAWORLD.mergeGroup=function(THREE,group,root){
  ...
  for(const sig in buckets){
    ...
    const nm=new THREE.MeshStandardMaterial({
      ...
      transparent:mat.transparent||false, opacity:mat.opacity??1});
    if(mat.transparent)nm.depthWrite=false;

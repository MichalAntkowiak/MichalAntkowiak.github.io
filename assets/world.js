...
MAWORLD.mergeStatic=function(THREE,root){
  root.updateMatrixWorld(true);
  const inv=new THREE.Matrix4().copy(root.matrixWorld).invert();
  const buckets={};const doomed=[];const emMats={};
  root.traverse(o=>{
    if(!o.isMesh||o.isInstancedMesh)return;
    ...
    let key;
    const hasEm=m.emissive&&(m.emissive.r+m.emissive.g+m.emissive.b>0.01);
    if(o.userData.win&&m.map)key='win';
    else if(m.map)return;                       // pojedyncze teksturowane (znak) zostają
    else if(hasEm)key='em|'+m.emissive.getHexString()+'|'+((m.emissiveIntensity??1).toFixed(2));
    else if(m.transparent&&(m.opacity==null||m.opacity>=0.98))key='glass';  // „szkło" lite → scalane opaque
    else if(m.transparent)return;               // mgiełki/smugi/aury (opacity<1) zostają osobno
    else if(m.isMeshBasicMaterial)key='basic';
    else key='std';
    if(hasEm){(emMats[key]=emMats[key]||m)}
    const g=o.geometry.clone().toNonIndexed();
    g.applyMatrix4(new THREE.Matrix4().multiplyMatrices(inv,o.matrixWorld));
    const n=g.attributes.position.count;
    if(key!=='win'){
      if(g.attributes.color){
        // geometria ma własne vertex-colors (np. planeta, ściana) — ZACHOWAJ
      }else{
        const col=new Float32Array(n*3);
  ...
  for(const k in buckets){
    if(k.indexOf('em|')!==0)continue;
    const src=emMats[k];
    const m=new THREE.MeshStandardMaterial({vertexColors:true,
      roughness:src.roughness??.5,metalness:src.metalness??0,
      emissive:src.emissive.clone(),emissiveIntensity:src.emissiveIntensity??1});
    root.add(new THREE.Mesh(concat(buckets[k]),m))}
};

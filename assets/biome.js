...
root.MABIOME={run(BIOME){
  ...
  const gw=BIOME.build(THREE);
  const ground=gw.group;world.add(ground);
  ground.position.y=GROUND_FINAL-R;
  MAWORLD.mergeStatic(THREE,ground);
  ...
  function frame(){
    ...
    if(gw.animate)gw.animate(dt,t,p);

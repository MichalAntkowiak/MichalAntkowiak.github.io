...
root.MABIOME_RIDGE={
  ...
  build(THREE){
    ...
    function ramp(x1,z1,y1,x2,z2,y2){
      ...
      // ciągła poręcz po zewnętrznej
      const rail=new THREE.Mesh(new THREE.BoxGeometry(L+3.4,.14,.14),WHITE);
      rail.position.set((x1+x2)/2+nx*2.8,(y1+y2)/2+1.28,(z1+z2)/2+nz*2.8);
      rail.rotation.y=-Math.atan2(dz,dx);
      rail.rotation.z=Math.atan2(dy,L);G.add(rail)}
    ...
    /* — GŁAZY i KĘPY TRAW na stoku — */
    for(let i=0;i<16;i++){
      const gx=(rnd()-.5)*380, gz=-20-rnd()*150;
      if(T.some(function(tt){return Math.hypot(gx-tt[0],gz-tt[1])<18}))continue;
      const gy=slopeH(gz);
      const s2=1+rnd()*2.4;
      const g2=new THREE.IcosahedronGeometry(1,0);
      const P2=g2.attributes.position;
      for(let k2=0;k2<P2.count;k2++)P2.setXYZ(k2,P2.getX(k2)*(0.7+rnd()*.5),P2.getY(k2)*(0.5+rnd()*.4),P2.getZ(k2)*(0.7+rnd()*.5));
      g2.computeVertexNormals();
      const boul=new THREE.Mesh(g2,i%2?ROCK2:std(0x9a7a62,.9));
      boul.scale.setScalar(s2);boul.position.set(gx,gy+s2*.3,gz);
      boul.rotation.y=rnd()*Math.PI;G.add(boul)}
    for(let i=0;i<40;i++){
      const gx=(rnd()-.5)*300, gz=20+rnd()*36;   // łąka
      for(let b2=0;b2<3;b2++){
        const bl=new THREE.Mesh(new THREE.ConeGeometry(.09,.8+rnd()*.5,4),
          b2%2?GREENT:std(0x8a9a52,.9));
        bl.position.set(gx+(rnd()-.5)*.7,.4,gz+(rnd()-.5)*.7);
        bl.rotation.z=(rnd()-.5)*.35;G.add(bl)}}

    /* — PTAKI o świcie (3, krążą) — */
    const birds=[];
    for(let i=0;i<3;i++){
      const bird=new THREE.Group();bird.userData.dyn=true;G.add(bird);
      for(const sgn of[-1,1]){const w2=new THREE.Mesh(new THREE.ConeGeometry(.32,1.0,3),std(0x2c2430,.8));
        w2.rotation.z=sgn*Math.PI/2;w2.position.x=sgn*.42;bird.add(w2)}
      bird.userData={ph:i*2.4,r:34+i*10,h:46+i*10,cx:-10,cz:-70};birds.push(bird)}
    ...
    return {group:G, pad:{h:.07},
      animate(dt,t){
        ...
        birds.forEach(function(b3){const u=b3.userData;u.ph+=dt*.5;
          b3.position.set(u.cx+Math.cos(u.ph)*u.r,u.h+Math.sin(u.ph*1.7)*3,u.cz+Math.sin(u.ph)*u.r);
          b3.rotation.y=-u.ph;
          b3.children.forEach(function(w3,k3){w3.rotation.x=Math.sin(t*9+k3)*.5})});
      }};
  }
};
})(window);

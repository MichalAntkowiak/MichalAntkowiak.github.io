...
root.MABIOME_RIDGE={
  ...
  build(THREE){
    ...
    const BASE = makePlanet(0,-7.44,0,7.5,0x4ea87a,0x3a2f4e);   // biegun N ≈ 0.06, tafla padu ≈ 0.18          // pad
    ...
    /* ── PIERŚCIEŃ wokół planety studiów (jak Saturn) ── */
    {const ring=new THREE.Mesh(new THREE.TorusGeometry(P4.R*1.7,.5,4,40),
       new THREE.MeshStandardMaterial({color:0x9a8ad0,roughness:.6,transparent:true,opacity:.55,depthWrite:false}));
     ring.rotation.x=Math.PI/2.25;ring.rotation.z=.3;ring.scale.y=.25;
     P4.group.add(ring);
     const ring2=new THREE.Mesh(new THREE.TorusGeometry(P4.R*2.05,.22,4,40),
       new THREE.MeshStandardMaterial({color:0xc9baf0,roughness:.6,transparent:true,opacity:.35,depthWrite:false}));
     ring2.rotation.copy(ring.rotation);ring2.scale.y=.25;P4.group.add(ring2)}

    /* ── RÓŻA POD KLOSZEM na planecie bazowej (ukłon w stronę Małego Księcia) ── */
    {const rose=new THREE.Group();
     const stem=new THREE.Mesh(new THREE.CylinderGeometry(.03,.04,.6,5),LEAF2);
     stem.position.y=.3;rose.add(stem);
     const bloom=new THREE.Mesh(new THREE.IcosahedronGeometry(.16,0),std(0xd8305a,.5));
     bloom.position.y=.66;rose.add(bloom);
     for(let k=0;k<3;k++){const pet=new THREE.Mesh(new THREE.SphereGeometry(.09,6,4),std(0xe8507a,.5));
       const a=k/3*Math.PI*2;pet.position.set(Math.cos(a)*.1,.62,Math.sin(a)*.1);rose.add(pet)}
     const lf=new THREE.Mesh(new THREE.BoxGeometry(.16,.02,.08),LEAF);
     lf.position.set(.09,.32,0);lf.rotation.z=-.5;rose.add(lf);
     const dome=new THREE.Mesh(new THREE.SphereGeometry(.34,10,8,0,Math.PI*2,0,Math.PI/2),
       new THREE.MeshStandardMaterial({color:0xcfe4f2,roughness:.15,metalness:.1,transparent:true,opacity:.28,depthWrite:false}));
     dome.position.y=.44;rose.add(dome);
     const rim=new THREE.Mesh(new THREE.TorusGeometry(.34,.025,6,14),SLATE);
     rim.rotation.x=Math.PI/2;rim.position.y=.44;rose.add(rim);
     placeOnSurface(BASE.group,BASE.R,55,-120,rose)}

    /* ── KRATERY (ciemniejsze niecki na planetach) ── */
    function crater(pl,lat,lon,s2){
      const c=new THREE.Group();
      const dish=new THREE.Mesh(new THREE.CylinderGeometry(.5*s2,.62*s2,.12,10),
        std(0x000000,.95));dish.material.color.copy(new THREE.Color(0x000000));
      dish.material=new THREE.MeshStandardMaterial({color:0x3a3a52,roughness:.95});
      dish.position.y=.02;c.add(dish);
      const rim2=new THREE.Mesh(new THREE.TorusGeometry(.56*s2,.06*s2,5,12),
        new THREE.MeshStandardMaterial({color:0x6a6a8a,roughness:.9}));
      rim2.rotation.x=Math.PI/2;rim2.position.y=.08;c.add(rim2);
      placeOnSurface(pl.group,pl.R,lat,lon,c)}
    crater(P1,-15,30,1);crater(P2,45,-150,1.2);crater(P2,-35,80,.8);
    crater(P4,-25,40,1);crater(BASE,15,-160,1.1);

    /* ── KSIĘŻYC planety kuriera (orbituje) ── */
    const moon=dyn(new THREE.Mesh(new THREE.IcosahedronGeometry(1,1),
      new THREE.MeshStandardMaterial({color:0xb8b8c8,roughness:.95,flatShading:true})));
    moon.userData={ph:0};G.add(moon);
    {const u=moon.userData;moon.position.set(24+Math.cos(0)*11,112+2,-24+Math.sin(0)*11)}

    /* ── SPADAJĄCE GWIAZDY (dwie, cyklicznie) ── */
    const shooting=[];
    for(let i=0;i<2;i++){
      const st=dyn(new THREE.Group());
      const head=new THREE.Mesh(new THREE.SphereGeometry(.25,6,5),bas(0xffffff));st.add(head);
      for(let k=1;k<=4;k++){const seg=new THREE.Mesh(new THREE.SphereGeometry(.2-k*.035,5,4),
        glow(0xdfe8ff,.5-k*.1));seg.position.x=k*.9;st.add(seg)}
      st.userData={ph:i*7,per:11+i*4,x0:60-i*160,y0:250-i*40,z0:-90};
      st.visible=false;G.add(st);shooting.push(st)}

    /* ── BEACON przy padzie (mrugający) ── */
    const beacon=dyn(new THREE.Mesh(new THREE.SphereGeometry(.14,8,6),bas(0xff6a3a)));
    beacon.position.set(2.4,BASE.R-7.44+.5,0);   // przy krawędzi padu (świat lokalny)
    G.add(beacon);
    ...
    return {group:G, pad:{h:.18},
      animate(dt,t){
        ...
        comets.forEach(c=>{const u=c.userData;u.ph=(u.ph+dt*u.sp)%1;
          ...
          c.lookAt(nx)});
        // księżyc P2 orbituje
        {const u=moon.userData;u.ph+=dt*.5;
         moon.position.set(24+Math.cos(u.ph)*11,112+Math.sin(u.ph*.7)*2,-24+Math.sin(u.ph)*11);
         moon.rotation.y+=dt*.4}
        // spadające gwiazdy: krótkie przeloty
        shooting.forEach(st=>{const u=st.userData;
          const tt=(t+u.ph)%u.per;
          if(tt<1.4){st.visible=true;
            st.position.set(u.x0-tt*60,u.y0-tt*26,u.z0);
            st.rotation.z=.42}

...
root.MABIOME_FACTORY={
  ...
  build(THREE){
    ...
    for(const [lx,lz] of [[-40,8],[8,6],[-30,-30],[30,-24],[62,22],[-70,26]]){
      const p=new THREE.Mesh(new THREE.CylinderGeometry(.12,.16,5.4,7),IRON);
      p.position.set(lx,2.7,lz);G.add(p);
      const arm=new THREE.Mesh(new THREE.BoxGeometry(1.3,.12,.12),IRON);
      arm.position.set(lx+.6,5.3,lz);G.add(arm);
      const gl=new THREE.Mesh(new THREE.SphereGeometry(.3,8,6),bas(0xffd98a));
      gl.position.set(lx+1.2,5.1,lz);G.add(gl);lampGlows.push(gl)}

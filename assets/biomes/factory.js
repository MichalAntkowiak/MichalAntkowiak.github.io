...
root.MABIOME_FACTORY={
  ...
  build(THREE){
    ...
    {const floor2=new THREE.Mesh(new THREE.CylinderGeometry(TT.r-.2,TT.r+.2,.3,28),std(0x4a4038,.9));
     ...
     const wall=new THREE.Mesh(new THREE.TorusGeometry(TT.r,.5,8,28),std(0x4e463e,.7,.3));
    ...
    function loco(cabCol){
      ...
      for(const bx of[-2.4,-1.0,.6]){const band=new THREE.Mesh(new THREE.CylinderGeometry(1.39,1.39,.16,14),BRASS);
        band.rotation.z=Math.PI/2;band.position.set(bx,2.75,0);L.add(band)}
      for(const sg of[-1,1]){const rail3=new THREE.Mesh(new THREE.CylinderGeometry(.045,.045,5.6,5),BRASS);
        rail3.rotation.z=Math.PI/2;rail3.position.set(-1.1,3.85,sg*1.42);L.add(rail3)}
      const cab=new THREE.Mesh(new THREE.BoxGeometry(2.6,2.9,2.9),cabCol);
      cab.position.set(3.2,3.05,0);L.add(cab);
      const sill=new THREE.Mesh(new THREE.BoxGeometry(2.7,.14,3.0),BRASS);
      sill.position.set(3.2,2.3,0);L.add(sill);
      const winB=new THREE.Mesh(new THREE.BoxGeometry(.09,.9,1.5),WARM);
      winB.position.set(4.46,3.5,0);L.add(winB);
    ...
    for(let k=0;k<5;k++){
      ...
      const roof2=new THREE.Mesh(new THREE.BoxGeometry(10.4,.4,17.8),BRICK2);
      roof2.position.set(0,8.25,-2.5);st.add(roof2);
      const roofTrim=new THREE.Mesh(new THREE.BoxGeometry(10.8,.18,.5),IRON);
      roofTrim.position.set(0,8.3,6.3);st.add(roofTrim);
      const skyM=new THREE.MeshStandardMaterial({color:0xffe2b0,roughness:.3,metalness:.1,
        emissive:new THREE.Color(0xd08a40),emissiveIntensity:.55});
      const sky=new THREE.Mesh(new THREE.BoxGeometry(2.2,.22,13),skyM);
      sky.position.set(0,8.5,-2.8);st.add(sky);
      const ridge2=new THREE.Mesh(new THREE.BoxGeometry(2.6,.1,13.4),IRON);
      ridge2.position.set(0,8.66,-2.8);st.add(ridge2);
      track(TT.x+Math.sin(a)*(TT.r+1),TT.z-Math.cos(a)*(TT.r+1),sx+Math.sin(a)*4.5,sz-Math.cos(a)*4.5);
      const chim2=new THREE.Mesh(new THREE.CylinderGeometry(.4,.5,2.8,8),BRICK);
      chim2.position.set(3.2,9.8,-7);st.add(chim2);
      const chc=new THREE.Mesh(new THREE.CylinderGeometry(.55,.55,.3,8),IRON);
      chc.position.set(3.2,11.2,-7);st.add(chc)}
    ...
    /* ── DETAL TERENU: łaty, stos podkładów, beczki, trawy, nastawnia ── */
    for(let k=0;k<9;k++){
      const patch=new THREE.Mesh(new THREE.CylinderGeometry(3.5+rnd()*4.5,3.5+rnd()*4.5,.05,9),
        k%2?std(0x5c4a3e,.95):std(0x75604e,.95));
      patch.position.set((rnd()-.5)*220,.028,(rnd()-.5)*120-8);G.add(patch)}
    {const stx=-34,stz=-4;
     for(let k=0;k<4;k++){const tie=new THREE.Mesh(new THREE.BoxGeometry(4.6,.3,.5),WOODT);
       tie.position.set(stx,(k<2?.15:.48),stz+(k%2)*.6-((k>=2)?.3:0));tie.rotation.y=(k>=2)?.06:0;G.add(tie)}}
    for(const [bx,bz] of [[18,-6],[19.3,-6.6],[70,26]]){
      const drum=new THREE.Mesh(new THREE.CylinderGeometry(.55,.55,1.4,9),REDD);
      drum.position.set(bx,.7,bz);G.add(drum);
      const lid=new THREE.Mesh(new THREE.CylinderGeometry(.57,.57,.1,9),IRON);
      lid.position.set(bx,1.42,bz);G.add(lid)}
    for(let k=0;k<10;k++){
      const gx=(rnd()-.5)*220, gz=(rnd()-.5)*120-6;
      for(let b2=0;b2<3;b2++){const bl=new THREE.Mesh(new THREE.ConeGeometry(.06,.7+rnd()*.5,4),std(0x7a7040,.9));
        bl.position.set(gx+(rnd()-.5)*.5,.35,gz+(rnd()-.5)*.5);bl.rotation.z=(rnd()-.5)*.4;G.add(bl)}}
    {const nb=new THREE.Group();nb.position.set(14,0,-26);nb.rotation.y=.4;G.add(nb);
     const base2=new THREE.Mesh(new THREE.BoxGeometry(3.4,2.2,2.8),BRICK);base2.position.y=1.1;nb.add(base2);
     const up=new THREE.Mesh(new THREE.BoxGeometry(3.8,1.8,3.2),WOODT);up.position.y=3.1;nb.add(up);
     const win2=new THREE.Mesh(new THREE.BoxGeometry(3.5,.9,3.3),WARM);win2.position.y=3.2;nb.add(win2);
     const rf=new THREE.Mesh(new THREE.ConeGeometry(2.9,1.1,4),IRON);rf.position.y=4.55;rf.rotation.y=Math.PI/4;nb.add(rf);
     const fin=ironLily(.8,PATINA2);fin.position.y=5.1;nb.add(fin)}
